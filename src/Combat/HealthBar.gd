	...
	if health < health_over.max_value * danger_zone:
		if will_pulse:
			if !pulse_tween.is_active():
				...
				# Signal will be emitted here in order to signal for sound effect when health is low
				# interpolate_callback function will call a method everytime between runs 
...
func _on_max_health_updated(max_health):
	...
	health_over.step = max_health / 100
	health_under.max_value = max_health
	health_under.step = max_health / 100
